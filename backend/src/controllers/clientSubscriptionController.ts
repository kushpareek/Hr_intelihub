...
// @access  Admin
export const getClientSubscriptions = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
...
// @access  Admin
export const getClientSubscriptionById = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const subscriptionId = req.params.id;
    const result = await query('SELECT * FROM client_subscriptions WHERE id = $1', [subscriptionId]);
    if (result.rows.length === 0) {
      res.status(404).json({ message: 'Client subscription not found' });
      return;
    }
...
// @access  Admin
export const createClientSubscription = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!client_name || !client_email || !plan || !status || !joined_date) {
      res.status(400).json({ message: 'Client name, email, plan, status, and joined date are required' });
      return;
    }
    ...
    if (emailCheck.rows.length > 0) {
        res.status(400).json({ message: 'Client email already exists for a subscription.' });
        return;
    }
...
// @access  Admin
export const updateClientSubscription = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const subscriptionId = req.params.id;
    const { client_name, client_email, plan, status, joined_date, renewal_date } = req.body;
    ...
    if (client_email && client_email !== existingSubscription.rows[0].client_email) {
        ...
        if (emailCheck.rows.length > 0) {
            res.status(400).json({ message: 'New client email already exists for another subscription.' });
            return;
        }
    }
...
// @access  Admin
export const deleteClientSubscription = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const subscriptionId = req.params.id;
    ...
    if (relatedQueries.rows.length > 0) {
        res.status(400).json({ message: 'Cannot delete subscription. It has related support queries. Please reassign or delete them first.' });
        return;
    }
    ...
    if ((error as any).code === '23503') {
         res.status(400).json({ message: 'Cannot delete subscription. It is referenced by other records (e.g., support queries). Please remove those references first.' });
         return;
    }
